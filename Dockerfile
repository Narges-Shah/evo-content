...
COPY . /opt/evo-content

RUN \
  pip3 install -r /opt/evo-content/docs/requirements.txt \
    && \
...
RUN \
  chgrp --recursive user /opt/evo-content \
    && \
  chmod --recursive g+rwx /opt/evo-content \
    && \
  echo "user granted permissions to /opt/evo-content"
